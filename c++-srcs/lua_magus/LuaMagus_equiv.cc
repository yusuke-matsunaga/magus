...
)
{
  ...
  if ( n == 2 || n == 3 ) {
	...
	mgr.set_loglevel(loglevel);
      ...
      lua_Integer loop_limit;
      ret = lua.get_int_field(3, "loop_limit", loop_limit);
      if ( ret == Luapp::ERROR ) {
	return lua.error_end("Error in equiv(): Illegal value for 'loop_limit' in 3rd argument.");
      }
      if ( ret == Luapp::OK ) {
	mgr.set_loop_limit(loop_limit);
      }
    }

    // 等価検証を行う．
    auto result = mgr.check(*net1, *net2);
    // 結果をスタックに積む．
