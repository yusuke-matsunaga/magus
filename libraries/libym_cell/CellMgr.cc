...
CellMgr::load_library(istream& s)
{
  ...
  mFuncNum = BinIO::read_32(s);
  {
    ...
    mFuncArray = new (p) CellFuncGroup[mFuncNum];
  }
  for (ymuint i = 0; i < mFuncNum; ++ i) {
    CellFuncGroup& func = mFuncArray[i];
  ...
  mRepNum = BinIO::read_32(s);
  {
    ...
    mRepArray = new (p) CellFuncClass[mRepNum];
  }
  ...
  mFFGroupNum = BinIO::read_32(s);
  {
    ...
    mFFGroupArray = new (p) CellFFGroup[mFFGroupNum];
  }
  for (ymuint i = 0; i < mFFGroupNum; ++ i) {
    CellFFGroup& ff_group = mFFGroupArray[i];
  ...
  mFFClassNum = BinIO::read_32(s);
  {
    ...
    mFFClassArray = new (p) CellFFClass[mFFClassNum];
  }
  for (ymuint i = 0; i < mFFClassNum; ++ i) {
    CellFFClass& ff_class = mFFClassArray[i];
    ff_class.mBits = BinIO::read_8(s);
    ymuint ng = BinIO::read_32(s);
    ff_class.mGroupNum = ng;
    {
      void* p = mAlloc.get_memory(sizeof(const CellFFGroup*) * ng);
      ff_class.mGroupList = new (p) const CellFFGroup*[ng];
    }
...
CellMgr::load_repfunc(istream& s,
		      ymuint id)
{
  CellFuncClass& rep = mRepArray[id];
  ymuint n = BinIO::read_32(s);
  rep.mFuncNum = n;
  if ( n > 0 ) {
    {
     ...
     const CellMgr& cell_mgr)
{
  ...
  // 関数情報の出力
  const CellFuncGroup& func0 = cell_mgr.const0_func();
  const CellFuncGroup& func1 = cell_mgr.const1_func();
  const CellFuncGroup& func2 = cell_mgr.buf_func();
  const CellFuncGroup& func3 = cell_mgr.inv_func();
  ymuint nf = cell_mgr.func_num();
  for (ymuint i = 0; i < nf; ++ i) {
    const CellFuncGroup& func = cell_mgr.func_group(i);
    if ( &func == &func0 ) {
      s << "Const0" << endl;
    }
    else if ( &func == &func1 ) {
  ...
  for (ymuint i = 0; i < nr; ++ i) {
    const CellFuncClass& rep = cell_mgr.rep(i);
    s << "Rep#" << i << endl;
    s << "  equivalence = ";
  ...
  for (ymuint i = 0; i < nff; ++ i) {
    const CellFFClass& ff_class = cell_mgr.ff_class(i);
    s << "FF-Class#" << i << endl;
    s << "  CLOCK: ";
