...
const CellLibrary*
MislibMgr::gen_library(const string& lib_name,
		       const MislibPt* root)
{
  bool error = false;
  ...
  for (const MislibPt* gate = root->top(); gate; gate = gate->next()) {
  ...
  // セルの内容の設定
  ymuint cell_id = 0;
  for (const MislibPt* gate = root->top(); gate;
    ...
    for (ymuint i = 0; i < ni; ++ i) {
      ...
      case kNoninv:  sense = kSensePosiUnate; break;
      case kInv:     sense = kSenseNegaUnate; break;
      case kUnknown: sense = kSenseNonUnate; break;
