...
{
  Bdd f = mBddMgr.make_zero();
  tPol pol = val ? kPolNega : kPolPosi;
  tDgEdge result = new_edge(f, BddVarSet(mBddMgr), kC0,
...
DgMgr::make_literal(tVarId varid,
		    tPol pol)
{
  Bdd f = mBddMgr.make_posiliteral(varid);
  tDgEdge result = new_edge(f, f.support(), kLitP,
			    vector<tDgEdge>(0), pol);
  return result;
}
...
// @param[in] type ノードタイプ(ここでは kOr か kXor)
// @param[in] inputs 入力枝のリスト
// @param[in] opol 出力極性
// @return 生成された枝
inline
tDgEdge
DgMgr::make_node(tType type,
		 const vector<tDgEdge>& inputs,
		 tPol opol)
{
  if ( type == kOr ) {
...
DgMgr::decomp_step(tBddEdge e)
{
  ...
  if ( p != node_hash.end() ) {
    ...
    tBddEdge e0;
    tBddEdge e1;
    tVarId varId = mBddMgr.mImpl->root_decomp(e, e0, e1);
	     ...
	     tDgEdge r1)
{
  ...
  Bdd F = mBddMgr.make_bdd(varid, F0, F1);

  if ( mBidecomp ) {
    // 二項分解の時はここで終わり．
    BddVarSet support = v0->support() + v1->support() + BddVarSet(mBddMgr, varid);
  ...
  if ( v0->type() == kCplx && v1->type() == kCplx ) {
    if ( n0 == nc + 1 && n1 == nc + 1 ) {
  ...
  if ( v0->type() == kCplx && !sup0diff.empty() && sup1diff.empty() ) {
    for (ymuint i = 0; i < n0; ++ i) {
      DgNode* inode = v0->input_node(i);
  ...
  if ( v1->type() == kCplx && !sup1diff.empty() && sup0diff.empty() ) {
    for (ymuint i = 0; i < n1; ++ i) {
...
DgMgr::sweep()
{
  ...
  while ( p != mNodeHash1.end() ) {
    ...
    if ( mBddMgr.mImpl->check_noref(e) ) {
	       ...
	       tPol opol)
{
  ...
  tDgEdge result = new_edge(g, support, kOr, inputs2, opol);
  return result;
}
	       ...
	       tPol opol)
{
  ...
  ymuint new_n = inputs.size();
  if ( new_n == 1 ) {
    // 1入力ならノードを新たに作る必要はない．
    return inv_pol(inputs[0], opol);
  }

  tDgEdge result = new_edge(g, support, kOr, inputs, opol);
  return result;
}
		...
		tPol opol)
{
  ...
  tDgEdge result = new_edge(g, support, kXor, inputs2, xpol * opol);
  return result;
}
		...
		tPol opol)
{
  ...
  tDgEdge result = new_edge(g, support, kXor, inputs, opol);
  return result;
}
		 ...
		 tPol opol)
{
  ...
  tDgEdge result = new_edge(f, support, kCplx, inputs2, opol);
  return result;
}
