			...
			ymuint g0)
{
  ...
  if ( g0 == conf0.ic_num() ) {
    ...
    for (vector<ymuint>::iterator q = top_list.begin();
	 q != top_list.end(); ++ q) {
      ymuint c = *q;
      NpnConf conf1(conf, g0, c);
      w2refine2(conf1, g0);
...
// @brief シグネチャが最大になるように極性と順序を決める．
void
NpnMgrImpl::tvmax_recur(NpnConf& conf,
			ymuint g0)
{
  for ( ; ; ) {
    if ( debug & debug_w2max ) {
