...
LitPOp::LitPOp(CNFddMgrImpl& mgr) :
  UniVOp(mgr)
{
}
...
// 下請け関数
CNFddEdge
LitPOp::apply_step(CNFddEdge f)
{
  // 特別な場合の処理
  // 1：fが0なら答は0，
  // 2: fが1なら答は level() の p枝に base を持つノード
  if ( f.is_zero() ) {
    return CNFddEdge::make_zero();
  }
  if ( f.is_one() ) {
    CNFddEdge zero = CNFddEdge::make_zero();
    return new_node(level(), zero, f, zero);
  }
  ...
  if ( f_level == level() ) {
    ...
    return new_node(f_level, e_p, e_0, e_n);
  }
