	     ...
	     vector<double>& failure_prob)
{
  ...
  if ( report_size ) {
    cout << "trans_rel.size() = " << size(trans_rel) << endl
	 << "err_trans_rel.size() = " << size(err_trans_rel) << endl
	 << "eq.size() = " << size(eq) << endl
	 << "ident.size() = " << size(ident) << endl;
  }

  // 回路対の基本の状態遷移関係
  Bdd trans_rel2 = trans_rel & err_trans_rel & cur_normal;
  if ( report_size ) {
    cout << "trans_rel2.size() = " << size(trans_rel2) << endl;
  }

  // 回路対の一時状態の状態遷移関係
  Bdd trans_rel3 = eq & ~ident & trans_rel2 & next_normal;
  if ( report_size ) {
    cout << "trans_rel3.size() = " << size(trans_rel3) << endl;
  }
  ...
  if ( report_size ) {
    cout << "trans_rel4.size() = " << size(trans_rel4) << endl;
  }

  // 次状態対が同一になる遷移関係
  Bdd trans_rel5 = (eq & ident & trans_rel2).esmooth(nvars) & next_ident_state;
  if ( report_size ) {
    cout << "trans_rel5.size() = " << size(trans_rel5) << endl;
  }
  ...
  if ( report_size ) {
    cout << "trans_rel_all.size() = " << size(trans_rel_all) << endl;
  }
