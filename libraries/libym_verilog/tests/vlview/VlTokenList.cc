...
///
/// $Id: VlTokenList.cc 2507 2009-10-17 16:24:02Z matsunaga $
///
...
VlTokenList::VlTokenList(QObject* parent) :
  QAbstractTableModel(parent)
{
}

// @brief デストラクタ
VlTokenList::~VlTokenList()
{
}

// @brief コラム数を返す．
int
VlTokenList::columnCount(const QModelIndex& /* parent */) const
{
  return 2;
}
...
// @brief 行数を返す．
int
VlTokenList::rowCount(const QModelIndex& /* parent */) const
{
  return mList.size();
}
...
// @brief 該当するデータを返す．
QVariant
VlTokenList::data(const QModelIndex& index,
		  int role) const
{
  if ( !index.isValid() ) {
    return QVariant();
...
const VlTokenList::FileRegion&
VlTokenList::loc(int row_pos)
{
  return mList.at(row_pos).mLoc;
}

VlTokenList::Token::Token(int id,
			  const FileRegion& file_region,
			  const char* str) :
  mId(id),
  mLoc(file_region),
