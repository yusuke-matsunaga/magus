...
TEST(SbjNodeTest, and1)
{
  ...
  int id = 1024;
  SbjNode* node = new SbjNode(id, SbjNodeType::And, ihandle1, ihandle2);

  EXPECT_EQ( id, node->id() );
  EXPECT_EQ( SbjNodeType::And, node->type() );
  EXPECT_FALSE( node->is_input() );
  EXPECT_FALSE( node->is_output() );
...
TEST(SbjNodeTest, xor1)
{
  ...
  int id = 1024;
  SbjNode* node = new SbjNode(id, SbjNodeType::Xor, ihandle1, ihandle2);

  EXPECT_EQ( id, node->id() );
  EXPECT_EQ( SbjNodeType::Xor, node->type() );
  EXPECT_FALSE( node->is_input() );
  EXPECT_FALSE( node->is_output() );
  EXPECT_TRUE( node->is_logic() );
  EXPECT_FALSE( node->is_and() );
