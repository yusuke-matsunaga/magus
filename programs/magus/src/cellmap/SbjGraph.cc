...
/// @file magus/cellmap/SbjGraph.cc 
/// @brief SbjGraph の実装ファイル
/// @author Yusuke Matsunaga (松永 裕介)
///
/// $Id: SbjGraph.cc 1978 2009-02-06 12:29:16Z matsunaga $
///
...
void
SbjNode::scan_efo(hash_set<size_t>& mark) const
{
  if ( !is_unselected() ) {
    mark.insert(id());
    return;
...
SbjGraph::rsort(vector<SbjNode*>& node_list) const
{
  ...
  for (SbjNodeList::const_iterator p = mOutputList.begin();
    ...
    if ( inode->umark() ) continue; // たぶんないはす．
...
void
SbjNode::find_imm_dom(size_t order,
		      vector<size_t>& order_array)
{
  order_array[id()] = order;
  size_t nfo = mFanoutList.size();
  if ( nfo == 0 || pomark() ) {
    mImmDom = NULL;
  }
  else if ( nfo == 1 ) {
