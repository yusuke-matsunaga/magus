...
/// @file magus/cellmap/Cut.cc 
/// @brief Cut の実装ファイル
/// @author Yusuke Matsunaga (松永 裕介)
///
/// $Id: Cut.cc 1978 2009-02-06 12:29:16Z matsunaga $
///
...
calc_expr_for_node(const SbjNode* node,
		   const hash_map<size_t, LogExpr>& logmap)
{
  LogExpr ans;
  if ( node == NULL ) {
    ...
    hash_map<size_t, LogExpr>::const_iterator p = logmap.find(node->id());
    if ( p != logmap.end() ) {
      ans = p->second;
    }
    else {
...
Cut::expr() const
{
  ...
  hash_map<size_t, LogExpr> logmap;
  for (size_t i = 0; i < ni(); i ++) {
