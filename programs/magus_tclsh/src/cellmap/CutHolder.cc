...
/// @file magus/cellmap/CutHolder.cc
/// @brief CutHolder の実装ファイル
/// @author Yusuke Matsunaga (松永 裕介)
///
/// $Id: CutHolder.cc 1978 2009-02-06 12:29:16Z matsunaga $
///
  ...
  mCutList(sbjgraph->max_node_id())
{
  ...
  for (size_t i = 0; i < n; ++ i) {
    ...
    if ( node == dnode ) {
      for (size_t j = 1; j < dup_list.size(); ++ j) {
	for (CutListIterator p = cut_list.begin();
	     p != cut_list.end(); ++ p) {
	  Cut* cut = *p;
...
// カットの領域を確保する．
Cut*
CutHolder::alloc_cut(size_t ni)
{
  size_t size = sizeof(Cut) + (ni - 1) * sizeof(SbjNode*);
