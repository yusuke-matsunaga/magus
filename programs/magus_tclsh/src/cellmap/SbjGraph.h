#ifndef MAGUS_CELLMAP_SBJGRAPH_H
#define MAGUS_CELLMAP_SBJGRAPH_H

/// @file magus/cellmap/SbjGraph.h 
/// @brief SbjGraph のヘッダファイル
/// @author Yusuke Matsunaga (松永 裕介)
///
/// $Id: SbjGraph.h 1978 2009-02-06 12:29:16Z matsunaga $
///
...
private:
  ...
  /// @brief タイプを入力に設定する．
  void
  set_input(size_t subid);
  
  /// @brief タイプを出力に設定する．
  void
  set_output(size_t subid,
	     bool inv);
  ...
  void
  find_imm_dom(size_t order,
	       vector<size_t>& order_array);

  /// @brief 実効的なファンアウト数を計算する．
  void
  scan_efo(hash_set<size_t>& mark) const;
...
///
/// 入力ノード，出力ノード，binary ノードを分けてリストで持つ
...
inline
void
SbjNode::set_input(size_t subid)
{
  mFlags = static_cast<size_t>(kINPUT) | (subid << 4);
}

// タイプを出力に設定する．
inline
void
SbjNode::set_output(size_t subid,
		    bool inv)
{
  mFlags = static_cast<size_t>(kOUTPUT) | (subid << 4) | (inv << 3);
}
...
{
  mFlags = static_cast<size_t>(kLOGIC) | (fcode << 3);
}
...
{
  return static_cast<bool>((mFlags >> 3) & 1);
}
...
inline
const SbjNode*
SbjNode::fanin(size_t pos) const
{
  // pos の範囲チェックはしていない！！！
  return mFanins[pos & 1].from();
}

// @brief ファンインのノードを得る．
inline
SbjNode*
SbjNode::fanin(size_t pos)
{
  // pos の範囲チェックはしていない！！！
  return mFanins[pos & 1].from();
}
...
inline
const SbjEdge*
SbjNode::fanin_edge(size_t pos) const
{
  // pos の範囲チェックはしていない！！！
  return &mFanins[pos & 1];
}

// ファンインの枝を得る．
inline
SbjEdge*
SbjNode::fanin_edge(size_t pos)
{
  // pos の範囲チェックはしていない！！！
  return &mFanins[pos & 1];
}
...
inline
SbjNode*
SbjGraph::input(size_t id) const
{
...
inline
SbjNode*
SbjGraph::output(size_t id) const
{
...
#endif // MAGUS_CELLMAP_SBJGRAPH_H
