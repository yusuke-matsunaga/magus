...
// @brief 枝の順序付けを行う．
void
NlSolverDfs::ordering(const NlGraph& graph,
		     vector<const NlEdge*>& edge_list)
{
  ymuint ne = graph.max_edge_id();
		       ...
		       const NlGraph& graph)
{
  ...
  for ( ; ; ) {
    bool no_change = true;
    for (ymuint node_id = 0; node_id < graph.max_nodeid(); ++ node_id) {
      const NlNode* node = graph.node(node_id);
      const vector<const NlEdge*>& edge_list = node->edge_list();
      ymuint ne = edge_list.size();
      vector<NlEdge*> tmp_list;
      tmp_list.resere(ne);
      ymuint n0 = 0;
      ymuint n1 = 0;
      for (ymuint i = 0; i < ne; ++ i) {
	const NlEdge* edge = edge_list[i];
	switch ( mEdgeArray[edge->id()] ) {
	case 0:
	...
	if ( n1 == 0 ) {
	  if ( n0 == 1 ) {
	    // 唯一の枝を選ぶ．
	    const NlEdge* edge = edge_list[0];
	    mEdgeArray[edge->id()] = 1;
	    no_change = false;
	  }
	}
	else if ( n1 == 1 ) {
...
void
NlSolverDfs::calc_frontier(vector<const NlNode*>& frontier_nodes,
			  const NlEdge* edge,
			  ymuint max_node_id,
			  vector<ymuint>& del_list,
			  int& pos1,
...
bool
NlSolverDfs::update_frontier(FrontierInfo& fr,
			    const NlEdge* edge,
			    bool selected,
			    const NlGraph& graph)
{
