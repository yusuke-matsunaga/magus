  ...
  mAlloc(4096)
{
  ...
  for (ymuint i = 0; i < mNodeNum; ++ i) {
    ...
    nf += node->fanin_num() * 2 + 2;
  }
  void* p = mAlloc.get_memory(sizeof(TpgFault) * nf);
  mFaultChunk = new (p) TpgFault[nf];
  ymuint fid = 0;
  for (ymuint i = 0; i < mNodeNum; ++ i) {
    TpgNode* node = &mNodeArray[i];
    node->mOutputFault[0] = new_fault(node, true, 0, 0, fid);
    node->mOutputFault[1] = new_fault(node, true, 0, 1, fid);
    ymuint ni = node->fanin_num();
    for (ymuint j = 0; j < ni; ++ j) {
      node->mInputFault[j * 2 + 0] = new_fault(node, false, j, 0, fid);
      node->mInputFault[j * 2 + 1] = new_fault(node, false, j, 1, fid);
    }
  }
		      ...
		      ymuint& fid)
{
  ...
  f->set(fid, node, is_output, ipos, val);
  ++ fid;
  return f;
}
