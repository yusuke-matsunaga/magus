...
/// @file SatEngineSingle3.cc
/// @brief SatEngineSingle3 の実装ファイル
...
// @brief コンストラクタ
SatEngineSingle3::SatEngineSingle3()
{
  mDopDummy = new_DopDummy();
  mTgGrasp = true;
  mUseDominator = true;
}
...
// @brief オプションを設定する．
void
SatEngineSingle3::set_option(const string& option_str)
{
...
void
SatEngineSingle3::run(TpgFault* fault,
		      TpgNode* fnode,
		      int fval,
		      ymuint max_id,
		      BackTracer& bt,
		      ...
		      UntestOp& uop)
{
  ...
  Bool3 ans = solve(solver, fault, bt, *mDopDummy, uop);
