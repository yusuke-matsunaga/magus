...
void
BtJust3::set_max_id(ymuint max_id)
{
...
void
BtJust3::clear_justified_hook(TpgNode* node)
{
...
// @param[in] model SATの値の割り当て結果を収めた配列
BtJust3::NodeList*
BtJust3::just_sub1(TpgNode* node,
		   const vector<Bool3>& model)
{
...
// @param[in] val 値
BtJust3::NodeList*
BtJust3::just_sub2(TpgNode* node,
		   const vector<Bool3>& model,
		   Bool3 val)
{
  ymuint ni = node->fanin_num();
...
// @brief 新しいリストのセルを返す．
BtJust3::NodeList*
BtJust3::new_list_cell(TpgNode* node)
{
...
// @brief リストをマージする．
void
BtJust3::list_merge(NodeList*& dst_list,
		    NodeList* src_list)
{
...
// @brief リストのサイズを返す．
ymuint
BtJust3::list_size(NodeList* node_list)
{
  ymuint n = 0;
...
// @brief リストを削除する．
void
BtJust3::list_free(NodeList* node_list)
{
