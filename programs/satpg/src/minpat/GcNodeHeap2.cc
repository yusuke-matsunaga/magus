...
// @brief コンストラクタ
// @param[in] num ノード数
GcNodeHeap2::GcNodeHeap2(ymuint num)
{
  mHeapSize = num;
  mNodeHeap = new GcNode2*[num];
  mNodeNum = 0;
}

// @brief デストラクタ
GcNodeHeap2::~GcNodeHeap2()
{
  delete [] mNodeHeap;
}
...
void
GcNodeHeap2::move_down(GcNode2* node)
{
  ...
  for ( ; ; ) {
    ...
    GcNode2* p_node = mNodeHeap[idx];
    GcNode2* l_node = mNodeHeap[l_idx];
    if ( r_idx == mNodeNum ) {
      // 右の子供を持たない時
	...
	locate_node(l_node, idx);
      ...
      // 左右の子供がいる場合
      GcNode2* r_node = mNodeHeap[r_idx];
      if ( compare(p_node, l_node) > 0 &&
...
// @param[in] node 対象のノード
void
GcNodeHeap2::move_up(GcNode2* node)
{
  ymuint idx = node->heap_location();
  if ( idx == 0 ) {
    // node はヒープに含まれない．
  ...
  -- idx;
  while ( idx > 0 ) {
    GcNode2* node = mNodeHeap[idx];
    ymuint p_idx = (idx - 1) / 2;
    GcNode2* p_node = mNodeHeap[p_idx];
...
// @brief 内容を出力する．
void
GcNodeHeap2::print(ostream& s) const
{
  s << " heap_size = " << mHeapSize << endl;
  for (ymuint i = 0; i < mNodeNum; ++ i) {
    const GcNode2* node1 = mNodeHeap[i];

    ASSERT_COND( node1->heap_location() - 1 == i );

    s << " GcNode#" << i << ": id = " << node1->id()
      << " SAT = " << node1->sat_degree() << endl;
  }
  s << endl;
}
