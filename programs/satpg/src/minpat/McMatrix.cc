...
// @param[in] node_array 列に対応するノードの配列
McMatrix::McMatrix(ymuint32 row_size,
		   ymuint32 col_size,
		   const GcNode* node_array) :
...
McMatrix::col_dominance()
{
  ...
  for (const McColHead* col1 = col_front();
	...
	pos2 = cell2->row_pos();
      ...
      if ( found ) {
#if 1
	if ( !check_dominance(col2->pos(), col1->pos()) ) {
	  // col2 の隣接条件が col1 の隣接条件よりも厳しければ支配しない．
	  continue;
	}
#endif
	// col2 は col1 を支配している．
...
// @brief ノードの隣接条件を比較する．
bool
McMatrix::check_dominance(ymuint id1,
			  ymuint id2)
{
  ...
  ymuint num1 = adj_list1.size();
  ymuint num2 = adj_list2.size();

#if 0
  if ( num1 > num2 ) {
    // adj_list1 の要素数が adj_list2 の要素数より多い
    // すくなくとも adj_list1 にあって adj_list2 にないものがある．
    return false;
  }
#endif
  ...
  for (ymuint i = 0; i < num1; ++ i) {
    GcNode* tmp1 = adj_list1[i];
#if 0
    if ( tmp1->is_deleted() ) {
      continue;
    }
#endif
    // tmp1 が adj_list2 にあるか検査
    bool found = false;
    for (ymuint j = 0; j < num2; ++ j) {
      if ( adj_list2[j] == tmp1 ) {
	found = true;
	break;
      }
  ...
  if ( not_found ) {
    // adj_list1 にあって adj_list2 にないものがある．
    return false;
  }
  return true;
#else
  TestVector* tv1 = node1->pat();
  TestVector* tv2 = node2->pat();
  if ( tv1->x_num() >= tv2->x_num() ) {
    return true;
  }
  return false;
#endif
